...
# 1. Компьютерная сеть и сетевое взаимодействие
...
# 6. Как происходит обращение к веб-сайту
...
# 8. Адресация на транспортном уровне: порты
...
# 24. ICMP и базовая диагностика сети
...
# 25. Как узнать сетевые параметры устройства
...
# 27. Сетевые угрозы с точки зрения модели CIA
...
# 30. Шифрование сетевого трафика
...
В рамках практического занятия были рассмотрены базовые принципы организации компьютерных сетей и основные протоколы, без понимания которых невозможно дальнейшее изучение методов защиты информации.

Модели OSI и TCP/IP позволяют представить сетевое взаимодействие как последовательность логических уровней. На канальном уровне используются кадры и MAC-адреса, на сетевом — IP-адреса и маршрутизация, на транспортном — TCP, UDP и порты, а на прикладном — DNS, HTTP, SSH и другие протоколы.

Отдельное внимание было уделено тому, как эти технологии связаны с информационной безопасностью. ARP и DNS необходимы для нормальной работы сети, однако при отсутствии дополнительных механизмов доверия могут становиться объектами подмены. Незашифрованный трафик может раскрывать передаваемые данные, а некорректная маршрутизация, фильтрация или сегментация способны расширить последствия инцидента.

Поэтому защита компьютерной сети строится комплексно. Она включает безопасную конфигурацию, сегментацию, межсетевые экраны, шифрование, аутентификацию, мониторинг и контроль сетевой инфраструктуры. Полученные знания будут необходимы в следующих занятиях при изучении криптографии, беспроводных сетей, TLS/HTTPS и практических методов веб-безопасности.
